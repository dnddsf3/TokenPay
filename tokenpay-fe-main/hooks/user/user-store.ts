...
// Define API Base URL
const API_BASE_URL = process.env.NEXT_API_BASE_URL || "http://localhost:7771";
...
interface UserStore {
  ...
  fetchUsers: () => Promise<void>;
   fetchUserById: (id: number) => Promise<UserRetrieveDTO | null>;
  createUser: (user: UserCreateDTO) => Promise<void>;
  createUserSignup: (user: UserCreateDTO) => Promise<void>;
...
export const useUserStore = create<UserStore>((set) => ({
  ...
  fetchUserById: async (id: number) => {
    ...
    try {
      ...
      const user: UserRetrieveDTO = await response.json();
      return user;
    } catch (error) {
    ...
    // Create User
  createUserSignup: async (user: UserCreateDTO) => {
    const authToken = Cookies.get("authToken"); // Retrieve token from cookies
    if (!authToken) {
      console.error("No credentials available. Please log in.");
      return;
