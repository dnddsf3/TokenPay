...
function SidebarSubmenu({ route, linkClicked }: ISidebarSubmenu) {
  ...
  return (
    <li className="relative px-6 py-3" key={route.name}>
      ...
      {isDropdownMenuOpen && (
        <span
          className="absolute h-12 inset-y-0 left-0 w-1 bg-red-600 rounded-tr-lg rounded-br-lg"
          aria-hidden="true"
        ></span>
      )}
      {/* Parent menu button */}
      ...
      >
        ...
        <DropdownIcon
          className={`w-4 h-4 transform transition-transform duration-150 ${isDropdownMenuOpen ? 'rotate-180' : ''
            }`}
          aria-hidden="true"
        />
      </button>
      ...
      {/* Dropdown menu */}
      <ul
        className={`p-2 mt-2 space-y-2 overflow-hidden text-sm font-medium text-gray-500 rounded-md shadow-inner bg-gray-50 dark:text-gray-400 dark:bg-gray-900 transition-all ${isDropdownMenuOpen ? 'max-h-xl opacity-100' : 'max-h-0 opacity-0'
          }`}
        aria-label="submenu"
      >
        {route.routes &&
